...
const MOCK_RESPONSES = [
  { keywords: ['hello', 'hi', 'hey'], response: "Hello! I am the MNEE AI Agent. I can help you manage your wallet, check gas prices, or automate transactions using MNEE stablecoin." },
  { keywords: ['help', 'what can you do', 'commands'], response: "Try these commands:\n• 'Check balance'\n• 'Gas price'\n• 'Monitor status'\n• 'My address'\n• 'Sign message'\n• 'Send 0.01 ETH to 0x...'\n• 'What is MNEE?'" },
  { keywords: ['subscription', 'paywall', 'cost', 'access'], response: "This is a decentralized paywall powered by MNEE stablecoin (0x8ccedbAe4916b79da7F3F612EfB2EB93A2bFD6cF). You pay MNEE tokens to unlock AI agent features. This demonstrates programmable payments for AI services." },
  { keywords: ['security', 'safe', 'private'], response: "I run entirely in your browser. I cannot access your private keys. You must approve all transactions manually in your wallet. All payments are made with MNEE, a USD-backed stablecoin on Ethereum." },
  { keywords: ['mnee', 'token', 'stablecoin', 'what is mnee'], response: "MNEE (0x8ccedbAe4916b79da7F3F612EfB2EB93A2bFD6cF) is a USD-backed stablecoin on Ethereum. It's designed for programmable money - enabling AI agents, creators, and businesses to automate payments and financial workflows." },
  { keywords: ['hackathon', 'project', 'about'], response: "This is a MNEE Hackathon submission demonstrating:\n• AI & Agent Payments (this AI interface)\n• Commerce & Creator Tools (subscription paywall)\n• Financial Automation (staking rewards)\n\nAll powered by MNEE stablecoin on Ethereum!" }
]
...
const getMockResponse = (input) => {
  ...
  return match ? match.response : `I didn't understand that. Try asking for "help" to see what I can do. Remember: I'm powered by MNEE, the programmable stablecoin!`
}
  ...
  const runAI = async () => {
    ...
    try {
      ...
      if (lowerInput.includes('balance') || lowerInput.includes('how much money')) {
        ...
        const text = `💰 Wallet Balance: ${formatEther(balance)} ETH\n\n💡 Tip: This AI agent accepts MNEE stablecoin for payments!`
      ...
      // 4. Network Info
        ...
        const text = `🌐 Connected to Chain ID: ${chainId} (${chainName})\n\nMNEE Token: 0x8ccedbAe4916b79da7F3F612EfB2EB93A2bFD6cF`
        setHistory(prev => [...prev, { role: 'ai', content: text }])
      }
      // 5. Check Address
      else if (lowerInput.includes('address') || lowerInput.includes('who am i')) {
        const text = `📍 Connected Address: ${address}\n\n🎯 You can use this address to receive MNEE tokens and interact with this AI agent.`
        setHistory(prev => [...prev, { role: 'ai', content: text }])
      }
      // 6. Sign Message
        ...
        const signature = await walletClient.signMessage({ message: "Verifying ownership for MNEE AI Agent - Hackathon Demo" })
        setHistory(prev => [...prev, { role: 'ai', content: `✅ Signature received:\n${signature.slice(0, 20)}...${signature.slice(-20)}\n\nYou've proven ownership of your wallet!` }])
      }
      // 7. Automate Payment (Send ETH)
        ...
        setHistory(prev => [...prev, { role: 'ai', content: `✅ Payment Automated Successfully!\nTransaction Hash: ${hash}\n\nThis demonstrates how AI agents can facilitate payments!` }])
      }
      // 8. Fallback to Mock Data
      else {
      ...
      setHistory(prev => [...prev, { role: 'ai', content: `❌ Error: ${error.message}\n\nPlease ensure you're connected to the correct network and have approved the transaction.` }])
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                <span className="text-indigo-400">✦</span> MNEE AI Agent
            </h2>
            <p className="text-slate-400 text-sm">Programmable Payments for AI Services</p>
        </div>
      ...
      <div className="space-y-4">
        <div className="space-y-4 max-h-96 overflow-y-auto mb-4 p-2 bg-slate-950/30 rounded-xl border border-slate-800/50">
          {history.length === 0 && !loading && (
            <div className="text-center text-slate-600 py-8 space-y-2">
              <p className="italic">No messages yet. Start the conversation!</p>
              <p className="text-xs">💡 Try: "What is MNEE?" or "Help"</p>
            </div>
          )}
          {history.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] p-3 rounded-xl ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-200 rounded-bl-none border border-slate-700'}`}>
                {msg.role === 'ai' && <strong className="text-indigo-400 text-xs uppercase tracking-wider mb-1 block">MNEE AI Agent</strong>}
        ...
        <textarea
            ...
            placeholder="Try: 'What is MNEE?', 'Check gas price', 'Send 0.001 ETH to 0x...', or 'Help'"
